  ...
  <form class="wrap create-top mt-[30px] max-md:mt-[50px] max-w-[1200px] w-full mx-auto max-xl:w-[90%] max-sm:w-[100%]">
    ...
    <div class="gap-x-0 gap-y-0 text-left max-xl:grid-cols-1 mx-auto max-sm:p-0 p-10 bg-[#FFFFFF]">
      ...
      <div class="px-2 bg-[#F9F9F9] border-b border-[#EFEFEF] h-[60px] flex items-center">
          ...
          <div class="flex-grow pt-2.5 font-semibold rounded-lg text-[16px] text-semibold overflow-hidden whitespace-nowrap text-ellipsis truncate">
            {{ title }}
          </div>
      </div>
      ...
      <div class="px-2 bg-[#F9F9F9] border-b border-t border-[#EFEFEF] h-[60px] flex items-center">
          <label
              class="width px-4 max-sm:px-2 text-[18px] after-small font-bold text-[#292929] whitespace-nowrap">카테고리</label>
          <div class="flex-grow flex gap-0">
            {{ category }}
          </div>
      </div>
      ...
      <div class="file-container px-2 bg-[#FFFFFF] border-b border-[#EFEFEF] flex items-center py-2">
        <label class="width px-4 max-sm:px-2 text-[18px] after-small font-bold text-[#292929] whitespace-nowrap py-2">
          파일 
        </label>
    ...
    <div class="max-w-[1190px] container mt-[64px] mb-[50px] max-sm:mt-[20px] ">
      <div class="max-w-[576px] flex gap-2 items-center justify-center max-sm:justify-end mx-auto">
        ...
        <button type="button" @click="goToEdit"
            class="px-16 max-sm:px-6 py-2.5 text-white bg-[#2B5BBB] rounded-[10px] hover:bg-[#648DDF]">
            수정하기
        </button>
        <button type="button" @click="cancel"
            class="px-16 max-sm:px-6 py-2.5 bg-[#F5F5F5] rounded-[10px] hover:bg-[#EBEBEB] border border-[#DBDEE3]">
            목록으로
        </button>
      </div>
    </div>
...
async function fetchPostData() {
  ...
  try {
    const { data, error } = await useFetch(`/api/admin/posts/${id}?board=qna`, {
...
// 🛠️ 기능 : 공지사항 수정하기 이동
const goToEdit = () => {
  router.push(`/admin/faq/edit/${route.params.id}`)
}
// 🛠️ 기능 : 돌아가기 버튼
function cancel() {
