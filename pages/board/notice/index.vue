      ...
      <div class="w-full gap-2 flex justify-end">
        <div class="left-content flex justify-end text-lg space-x-2 ">
          <button @click="goToCreate"
              class="bg-primaryBg text-primaryPoint text-[18px] max-sm:text-sm max-sm:py-2.5 max-sm:px-4 px-6 py-2 rounded-lg hover:bg-primaryBgH border border-primaryPoint focus:outline-none whitespace-nowrap">
              등록하기
          </button> 
        </div>
        <!-- right-content (탭들) -->
            ...
            <table class="w-full rounded-[12px] text-center bg-[#Fefefe] drop-shadow ">
                <thead>
                    <tr class="text-[18px] h-[60px]">
                        ...
                        <th class="px-2 py-2 font-semibold w-[15%]">구분</th>
                        <th class="px-2 py-2 font-semibold w-[15%]">공개여부</th>
                        <th class="px-2 py-2 font-semibold w-[30%]">제목</th>
                        <th class="px-2 py-2 font-semibold w-[15%]">등록일</th>
            ...
            <table id="reportTable" class="table-fixed w-full rounded-[12px] overflow-hidden text-center mt-4 bg-[#FEFEFE]">
              <tbody>
                ...
                <td class="px-2 py-2 w-[10%] text-[#727272]">{{ totalItems - ((currentPage - 1) * itemsPerPage + index) }}</td>
                  <td class="px-2 py-2 w-[15%] text-[#727272]">{{ post.category || '일반' }}</td>
                  <td class="w-[15%] text-primaryPoint">{{ post.public === 1 ? '공개' : '비공개' }}</td>
                  <td class="w-[30%] text-primaryPoint">
                    <RouterLink
                      :to="`/board/notice/view/${post.id}`"
                      class="font-semibold hover:underline text-primaryPoint"
                    >
                      {{ post.title }}
                    </RouterLink>
                  </td>
                  <td class="w-[15%] text-primaryPoint">{{ formatDate(post.updated_at) }}</td>
                  <td class="px-2 py-2 w-[15%]">
                    <div class="flex justify-center gap-2 px-2">
                      <BtnEdit :to="`/board/notice/edit/${post.id}`" />
...
// 🛠️ 기능 : 공지사항 등록하기 이동
const goToCreate = () => {
  router.push('/board/notice/create') 
}
